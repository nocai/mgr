...
func IsExistOfUser(user *User) (bool, error) {
	userId := user.Id
	// 设置Id = 0，方便查询
	user.Id = 0
	userSlice, err := FindUserByKey(&UserKey{User: user})
	if err != nil {
		beego.Error(err)
		return false, err
	}
...
func GetUserByUsername(username string) (*User, error) {
	users, err := FindUserByKey(&UserKey{User: &User{Username: username}})
	if err != nil {
		beego.Error(err)
		return nil, err
	}
...
func PageUser(key *UserKey) (*pager.Pager, error) {
